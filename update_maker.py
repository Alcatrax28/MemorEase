...
# Lecture de la version en mode DISTANT
def get_remote_info():
    try:
        if TEST_LOCAL:
...
# Définir que si version distante > locale = on a une nouvelle mise à jour disponible
...
def check_for_update():
    ...
    if is_update_available(local, remote_version):
        return {
            ...
            "madatory": remote_data.get("mandatory", False)
...
def download_update(url, log_callback=None, progress_callback=None, cancel_flag=None):
    try:
        ...
        # Vérifier que le fichier est suffisamment grand que pour être valide
        if os.path.getsize(temp_path) < 1024:
            if log_callback: log_callback("Fichier téléchargé corrompu ou invalide.")
            return None
